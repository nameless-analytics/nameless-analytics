CREATE OR REPLACE TABLE FUNCTION `tom-moretti.nameless_analytics.attribution_comparison`(start_date DATE, end_date DATE, conversion_name STRING, lookback_days INT64) AS (
  ...
  select
    ...
    session_source,
    split(session_campaign, '|')[safe_offset(0)] as session_campaign_year,
    split(session_campaign, '|')[safe_offset(1)] as session_campaign_country,
    split(session_campaign, '|')[safe_offset(2)] as session_campaign_funnel_stage,
    ...
    split(session_campaign, '|')[safe_offset(6)] as session_campaign_name,
    session_campaign,
    session_campaign_id,
