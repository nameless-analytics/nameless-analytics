...
with media_plan_data as (
    SELECT
      ...
      full_campaign_name as campaign_name,
      budget
  ...
  select 
    ...
    media_plan_data.campaign_name,
    split(media_plan_data.campaign_name, '|')[safe_offset(0)] as campaign_year,
    split(media_plan_data.campaign_name, '|')[safe_offset(1)] as campaign_country,
    split(media_plan_data.campaign_name, '|')[safe_offset(2)] as campaign_funnel_stage,
    split(media_plan_data.campaign_name, '|')[safe_offset(3)] as campaign_platform,
    split(media_plan_data.campaign_name, '|')[safe_offset(4)] as campaign_campaign_type,
    split(media_plan_data.campaign_name, '|')[safe_offset(5)] as campaign_marketing_objective,
  ...
  left join online_campaigns_performances
    ...
    and media_plan_data.campaign_name = online_campaigns_performances.campaign_name
);