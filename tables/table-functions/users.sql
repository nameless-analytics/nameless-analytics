...
with raw_user_data as (
  ...
  select
    ...
    date_diff(end_date, date(max(first_purchase_timestamp)), day) as days_from_first_purchase,
    date_diff(end_date, date(max(last_purchase_timestamp)), day) as days_from_last_purchase,
