...
with raw_session_data as (
    select
      ...
      countif(event_name = 'account_creation') as account_creation,
  ...
  select
    ...
    session_exit_page_title, 
    session_hostname,
    (case when account_creation >= 1 then 1 else 0 end) as session_with_account_creation,
    (case when newsletter_subscription >= 1 then 1 else 0 end) as session_with_newsletter_subscription,
    (case when form_submission >= 1 then 1 else 0 end) as session_with_form_submission,
